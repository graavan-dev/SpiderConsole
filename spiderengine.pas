...
procedure FireworksDisplay;
const
  Colors: array[0..5] of string = ('31', '32', '33', '34', '35', '36'); // red, green, yellow, blue, magenta, cyan
var
  x, y, c, i: Integer;
  msg: string;

  procedure PutXY(px, py: Integer; const S: string);
  begin
    Write(#27'[', py, ';', px, 'H', S);
  end;
...
begin
  ...
  for i := 1 to 1000 do
  begin
    x := Random(60) + 10;   // horizontal position
    y := Random(10) + 5;    // vertical position
    c := Random(6);         // color index

    // Center burst
    PutXY(x, y, #27'[' + Colors[c] + 'm' + '*' + #27'[0m');

    // Surrounding sparks
    PutXY(x-1, y,   #27'[' + Colors[c] + 'm' + '.' + #27'[0m');
    PutXY(x+1, y,   #27'[' + Colors[c] + 'm' + '.' + #27'[0m');
    PutXY(x,   y-1, #27'[' + Colors[c] + 'm' + '.' + #27'[0m');
    PutXY(x,   y+1, #27'[' + Colors[c] + 'm' + '.' + #27'[0m');
  ...
  PutXY(40 - (Length(msg) div 2), 22, msg);
